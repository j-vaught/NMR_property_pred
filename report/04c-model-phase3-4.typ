...
*Arrhenius-parameterized neural network.* A physics-informed model was tested that predicted the Arrhenius parameters $A$ and $B$ in @eq-arrhenius directly from NMR features, rather than predicting log viscosity at each temperature independently. This approach achieved a 66% average MAPE, worse than all other variants. The failure likely stems from the sensitivity of Arrhenius extrapolation to small errors in the fitted parameters: a modest error in $B$ translates into large viscosity errors at the extreme temperatures (233 K) where fuel predictions are validated.
